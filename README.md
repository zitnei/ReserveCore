...
| | URL |
|---|---|
| 🖥️ **デモサイト（フロントエンド）** | **https://reservecore-frontend.vercel.app** |
| ⚙️ API ヘルスチェック | https://reservecore-api.onrender.com/actuator/health |
| 💻 フロントエンドのソース | https://github.com/zitnei/reservecore-frontend |

**ログインしてすぐ予約フローを体験できます**（店舗・サービス・スタッフはデモデータを投入済み）。

| ロール | メールアドレス | パスワード | できること |
|---|---|---|---|
| 管理者 (ADMIN) | `admin@reservecore.com` | `admin1234` | 店舗・サービス登録、スタッフ割当、全予約閲覧 |
| スタッフ (STAFF) | `staff.sato@reservecore.com` | `password123` | 自店サービス登録、全予約閲覧・キャンセル |
| 顧客 (CUSTOMER) | `customer.tanaka@reservecore.com` | `password123` | 予約の作成・閲覧・キャンセル |

> 💡 自分で **新規登録**して顧客として試すこともできます（ホーム → 新規登録）。
>
> ⚠️ バックエンドは **Render Free プラン**で稼働しているため、**15分アクセスがないとスリープ**します。
> 初回アクセス時はコールドスタートで **30秒〜1分** 応答に時間がかかることがあります（2回目以降は高速）。
>
> ⚠️ これはポートフォリオ用のデモ環境です。上記の認証情報は公開デモ用であり、本番では環境変数で必ず変更します。

---

## 目次
- [ライブデモ](#-ライブデモブラウザですぐ触れます)
- [主なポイント](#主なポイント)
- [技術スタック](#技術スタック)
- [ER 図](#er-図)
- [API 一覧](#api-一覧)
...
| ✅ テスト | 実 PostgreSQL に対する**結合テスト 46 件**（権限・バリデーション・DB制約） |
...
### 店舗スタッフ `/api/stores/{storeId}/staff`
| Method | Path | 認証 | 説明 |
|---|---|---|---|
| POST | `/api/stores/{storeId}/staff` | **ADMIN** | ユーザーを自店スタッフに割当（CUSTOMER は STAFF に昇格） |
| GET | `/api/stores/{storeId}/staff` | 全ロール | スタッフ一覧（予約時の担当者選びに利用） |
| DELETE | `/api/stores/{storeId}/staff/{userId}` | **ADMIN** | スタッフ割当を解除（204） |
...
| ロール | できること |
|---|---|
| **ADMIN** | 全操作（店舗・サービス登録、スタッフ割当、全ユーザー閲覧、全予約の閲覧・キャンセル） |
...
curl -s -X POST http://localhost:8080/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{"email":"hanako@example.com","password":"password123","name":"佐藤花子"}'

# 5) 登録ユーザーの id を確認（ADMIN）→ 次の手順で使う userId を控える
curl -s http://localhost:8080/api/users -H "Authorization: Bearer $TOKEN"

# 6) そのユーザーを店舗1のスタッフに割当（CUSTOMER→STAFF に昇格）。userId は手順5の値
curl -s -X POST http://localhost:8080/api/stores/1/staff \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"userId":2}'
...
CUST=$(curl -s -X POST http://localhost:8080/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"taro@example.com","password":"password123"}' \
  | sed -E 's/.*"token":"([^"]+)".*/\1/')

# 8) 予約を作成（staffId は手順6のスタッフ id、日時は未来を指定）
curl -s -X POST http://localhost:8080/api/reservations \
  -H "Authorization: Bearer $CUST" -H "Content-Type: application/json" \
  -d '{"storeId":1,"serviceId":1,"staffId":2,"startTime":"2026-07-01T10:00:00"}'
```

> ✅ これで `register → 店舗 → サービス → スタッフ割当 → 予約` までを **API だけ**で再現できます。
> 同じスタッフ・同じ時間帯にもう一度予約すると `409`、`PATCH /api/reservations/{id}/cancel` でキャンセルできます。
> JWT はステートレスのため、**昇格したスタッフが STAFF 権限で操作するには再ログイン**が必要です。

---

## テスト

実 PostgreSQL に対して **結合テスト 46 件**を実行します（DB コンテナの起動が前提）。
...
| `StoreStaffControllerTest` | 9 | スタッフ割当 / CUSTOMER→STAFF昇格 / 権限・重複・404 / 昇格後に自店サービス作成 |
